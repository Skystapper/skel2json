...
namespace Skel2Json
{
    ...
    public class SkeletonBinaryReader
    {
        ...
        private SkeletonData skeletonData = new SkeletonData(); // Initialize to fix warning
            ...
            try
        {
            ...
            try
            {
                ...
                {
                    Console.WriteLine("Skipped 0 IK constraints");
                }
                for (int i = 0; i < n; i++)
                {
            ...
            try
            {
                ...
                {
                    Console.WriteLine("Skipped 0 Transform constraints");
                }
                for (int i = 0; i < n; i++)
                {
            ...
            {
                n = input.ReadInt(true);
                if (n > 0) Console.WriteLine($"Skipping {n} Path constraints");
            ...
            try
            {
                Console.WriteLine("Reading skins");
                // Default skin
                ...
                {
                    Console.WriteLine("Skipped 0 additional skins");
                }
                for (int i = 0; i < n; i++)
                {
                    try
            ...
            catch (Exception ex)
            {
                ...
                try
                {
                    ...
                    {
                        Console.WriteLine("Skipped 0 events");
                    }
                    for (int i = 0; i < n; i++) 
                    {
                        try 
            ...
            return skeletonData;
                    ...
                    {
                        Console.WriteLine("Skipped 0 animations");
                    }
                        ...
                        try
                        {
                            ...
                            try
                            {
                                ...
                                if (animation != null && animation.HasData())
                                {
                                    ...
                                    Console.WriteLine($"Completed animation: {animationName}");
                                }
                    ...
                    {
                        Console.WriteLine($"Successfully read {successfulAnimations} animations");
                    }
        ...
        private void ReadBoneTimelines(AnimationData animation)
        {
            try
            {
                ...
                // Read IK constraint timelines - Following exact SkeletonBinary.cs pattern
                int ikCount = input.ReadInt(true);
                
                if (ikCount > 0)
                {
                    animation.Ik = new Dictionary<string, IkTimelineData>();
                    ...
                    for (int i = 0; i < ikCount; i++)
                    {
                        ...
                        // Get IK constraint name - for now use index-based name
                        string ikName = $"ik_{ikIndex}";
                        if (ikIndex < skeletonData.Ik.Count)
                        {
                            ikName = skeletonData.Ik[ikIndex].Name;
                        }
                        
                        var ikTimeline = ReadIkTimeline(frameCount);
                        if (ikTimeline != null && ikTimeline.Count > 0)
                        {
                            var ikTimelineData = new IkTimelineData();
                            ikTimelineData.AddRange(ikTimeline);
                            animation.Ik[ikName] = ikTimelineData;
                        }
                    }
                }
                
                // Read Transform constraint timelines - Following exact SkeletonBinary.cs pattern  
                int transformCount = input.ReadInt(true);
                
                if (transformCount > 0)
                {
                    animation.Transform = new Dictionary<string, TransformTimelineData>();
                    ...
                    for (int i = 0; i < transformCount; i++)
                    {
                        ...
                        // Get Transform constraint name - for now use index-based name
                        string transformName = $"transform_{transformIndex}";
                        if (transformIndex < skeletonData.Transform.Count)
                        {
                            transformName = skeletonData.Transform[transformIndex].Name;
                        }
                        
                        var transformTimeline = ReadTransformTimeline(frameCount);
                        if (transformTimeline != null && transformTimeline.Count > 0)
                        {
                            var transformTimelineData = new TransformTimelineData();
                            transformTimelineData.AddRange(transformTimeline);
                            animation.Transform[transformName] = transformTimelineData;
                        }
                    }
                }
            }
            ...
            {
#if DEBUG
                Console.WriteLine($"Error reading bone/IK/transform timelines: {ex.Message}");
#endif
            }
        }
        ...
        private void ReadAttachmentTimelines(AnimationData animation)
        {
            try
        {
            // Note: IK and Transform timelines are now read in ReadBoneTimelines method
            // This method now only handles Path, Attachment, DrawOrder, and Event timelines
            ...
            // Read draw order timelines
            int drawOrderCount = input.ReadInt(true);
            if (drawOrderCount > 0)
            {
                animation.DrawOrder = ReadDrawOrderTimelines(drawOrderCount);
            }
            ...
            if (eventCount > 0)
            {
                animation.Events = ReadEventTimelines(eventCount);
            }
            }
            catch (Exception ex)
            {
        ...
        private SkinData? ReadSkin(bool defaultSkin)
        {
            ...
            {
                skin = new SkinData { Name = input.ReadStringRef() ?? "unknown" };
            ...
            for (int i = 0; i < slotCount; i++)
            {
                ...
                if (attachmentsCount > 0)
                {
                    ...
                    {
                        string attachmentName = input.ReadStringRef() ?? "unknown";
        ...
        private List<DeformKeyframeData> ReadDeformTimeline(int frameCount, int frameLast)
        {
            ...
            for (int frame = 0, bezier = 0; ; frame++)
            {
                ...
                if (end == 0)
                {
                    // Reset to base mesh - empty keyframe (only curve data)
                    // Don't set offset or vertices for empty keyframes
                    }
                else
                {
                    // Create deform array and read vertex data
                    int start = input.ReadInt(true);
                ...
                // Handle time property correctly - match original JSON format
                // First frame only omits time if it's exactly 0.0 and has no deformation data
                if (frame == 0 && Math.Abs(time) <= 0.001f && end == 0)
                {
        ...
        private List<DrawOrderTimelineData> ReadDrawOrderTimelines(int drawOrderCount)
        {
            var timeline = new List<DrawOrderTimelineData>();
            
            for (int frame = 0; frame < drawOrderCount; frame++)
            {
                try
                {
                    float time = input.ReadFloat();
                    ...
                    var keyframe = new DrawOrderTimelineData();
                    
                    // Handle time property - first frame at time=0 can omit time
                    if (frame == 0 && Math.Abs(time) <= 0.001f)
                    {
                        // First frame at time=0, omit time property by setting it to null
                        keyframe.Time = null;
                    }
                    else
                    {
                    ...
                    if (offsetCount > 0)
                    {
                        var offsets = new List<DrawOrderOffsetData>();
                        ...
                        for (int i = 0; i < offsetCount; i++)
                        {
                            ...
                            // Get slot name from slot lookup
                            string slotName = slotLookup.TryGetValue(slotIndex, out string name) ? name : $"slot_{slotIndex}";
                            
                            offsets.Add(new DrawOrderOffsetData
                            {
                                Slot = slotName,
                                Offset = offset
                            });
                    ...
                    // If offsetCount is 0, offsets will remain null (reset to setup pose)
                ...
                {
#if DEBUG
                    Console.WriteLine($"Error reading draw order frame {frame}: {ex.Message}");
#endif
                }
            }
        ...
        private List<EventTimelineData> ReadEventTimelines(int eventCount)
        {
            var timeline = new List<EventTimelineData>();
            
            for (int frame = 0; frame < eventCount; frame++)
            {
                try
                {
                    float time = input.ReadFloat();
                    ...
                    var keyframe = new EventTimelineData();
                    ...
                    // Get event name from event data index
                    string eventName = $"event_{eventDataIndex}";
                    if (eventDataIndex < skeletonData.Events.Count)
                    {
                        eventName = skeletonData.Events.Keys.ElementAt(eventDataIndex);
                    }
                    keyframe.Name = eventName;
                    
                    // Read event values - these override the defaults from event data
                    int intValue = input.ReadInt(false);
                    float floatValue = input.ReadFloat();
                    ...
                    // Only include values if they're different from defaults or non-zero
                    if (intValue != 0)
                    {
                        keyframe.Int = intValue;
                    }
                    
                    if (Math.Abs(floatValue) > 0.001f)
                    {
                        keyframe.Float = floatValue;
                    }
                    
                    if (!string.IsNullOrEmpty(stringValue))
                    {
                        keyframe.String = stringValue;
                    }
                    
                    // Check if event has audio properties
                    // We need to check the event data to see if it has audio path
                    if (eventDataIndex < skeletonData.Events.Count)
                    {
                        var eventData = skeletonData.Events.Values.ElementAt(eventDataIndex);
                        if (!string.IsNullOrEmpty(eventData.AudioPath))
                        {
                            // Read audio volume and balance
                            float volume = input.ReadFloat();
                            float balance = input.ReadFloat();
                ...
                {
#if DEBUG
                    Console.WriteLine($"Error reading event frame {frame}: {ex.Message}");
#endif
                }
            }
        ...
        private List<IkKeyframeData> ReadIkTimeline(int frameCount)
        {
            var timeline = new List<IkKeyframeData>();
            
            // Follow the exact pattern from official Spine SkeletonBinary.cs IK timeline reading
            // Read initial values: time, mix, softness, bendDirection, compress, stretch
            float time = input.ReadFloat();
            float mix = input.ReadFloat();
            float softness = input.ReadFloat() * scale; // Apply scale to softness
            
            for (int frame = 0, bezier = 0, frameLast = frameCount - 1; ; frame++)
            {
                // Read bendDirection as signed byte, compress and stretch as booleans
                sbyte bendDirection = input.ReadSByte();
                bool compress = input.ReadBoolean();
                bool stretch = input.ReadBoolean();
                
                var keyframe = new IkKeyframeData();
                
                // Only include mix if it's not the default value (1.0)
                if (Math.Abs(mix - 1.0f) > 0.001f)
                {
                    keyframe.Mix = mix;
                }
                
                // Only include softness if it's not zero
                if (Math.Abs(softness) > 0.001f)
                {
                    keyframe.Softness = softness;
                }
                
                // Only include bendDirection if it's not the default (1)
                // bendDirection: 1 = bendPositive true, -1 = bendPositive false
                if (bendDirection != 1)
                {
                    keyframe.BendPositive = bendDirection > 0;
                }
                
                // Only include compress if true
                if (compress)
                {
                    keyframe.Compress = true;
                }
                
                // Only include stretch if true  
                if (stretch)
                {
                ...
                if (frame == 0 && Math.Abs(time) <= 0.001f)
                {
                    keyframe.Time = null; // Omit time for first frame at 0
                }
                else
                {
                    keyframe.Time = time;
                ...
                switch (curveType)
                {
                    case 1: // CURVE_STEPPED
                        ...
                        break;
                    case 2: // CURVE_BEZIER
                        // IK timelines have 2 bezier curves (one for mix, one for softness)
                        // Each SetBezier call reads: cx1, cy1, cx2, cy2 (4 floats)
                        List<float> bezierCurve = new List<float>();
                        
                        // Read first SetBezier call (for mix)
                        float cx1_mix = input.ReadFloat();
                        float cy1_mix = input.ReadFloat();
                        float cx2_mix = input.ReadFloat();
                        float cy2_mix = input.ReadFloat();
                        
                        // Read second SetBezier call (for softness)
                        float cx1_soft = input.ReadFloat();
                        float cy1_soft = input.ReadFloat() * scale; // Apply scale to softness curve
                        float cx2_soft = input.ReadFloat();
                        float cy2_soft = input.ReadFloat() * scale; // Apply scale to softness curve
                        
                        // Combine both curves: [mix_curve, softness_curve]
                        bezierCurve.AddRange(new float[] { cx1_mix, cy1_mix, cx2_mix, cy2_mix, cx1_soft, cy1_soft, cx2_soft, cy2_soft });
        ...
        private List<TransformKeyframeData> ReadTransformTimeline(int frameCount)
        {
            var timeline = new List<TransformKeyframeData>();
            
            // Follow the exact pattern from official Spine SkeletonBinary.cs Transform timeline reading
            // Read initial values: time, mixRotate, mixX, mixY, mixScaleX, mixScaleY, mixShearY
            float time = input.ReadFloat();
            float mixRotate = input.ReadFloat();
            float mixX = input.ReadFloat();
            float mixY = input.ReadFloat();
            ...
            for (int frame = 0, bezier = 0, frameLast = frameCount - 1; ; frame++)
            {
                var keyframe = new TransformKeyframeData();
                
                // Set values following the original JSON format pattern
                keyframe.MixRotate = mixRotate;   // Always include
                keyframe.MixScaleX = mixScaleX;   // Always include  
                keyframe.MixShearY = mixShearY;   // Always include
                
                // Only include mixX if non-zero
                if (Math.Abs(mixX) > 0.001f)
                {
                    keyframe.MixX = mixX;
                }
                
                // mixY is typically omitted in the original JSON
                // Only include if different from mixX and non-zero
                if (Math.Abs(mixY) > 0.001f && Math.Abs(mixY - mixX) > 0.001f)
                {
                    keyframe.MixY = mixY;
                }
                
                // mixScaleY is typically omitted in the original JSON
                // Only include if different from mixScaleX and non-zero
                if (Math.Abs(mixScaleY) > 0.001f && Math.Abs(mixScaleY - mixScaleX) > 0.001f)
                {
                    keyframe.MixScaleY = mixScaleY;
                }
                ...
                switch (curveType)
                {
                    case 1: // CURVE_STEPPED
                        ...
                        break;
                    case 2: // CURVE_BEZIER
                        // Transform timelines have 6 bezier curves (one for each mix property)
                        // Each SetBezier call reads: cx1, cy1, cx2, cy2 (4 floats)
                        // So total: 8 floats for a 2D bezier curve
                        List<float> bezierCurve = new List<float>();
                        
                        // Read 6 SetBezier calls as per SkeletonBinary.cs
                        for (int curveIndex = 0; curveIndex < 6; curveIndex++)
                        {
                            ...
                            bezierCurve.AddRange(new float[] { cx1, cy1, cx2, cy2 });
                        }
    ...
    public class SkeletonInput
    {
        ...
        // Add methods to get stream position
